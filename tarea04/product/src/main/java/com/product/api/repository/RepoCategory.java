...
public interface RepoCategory extends JpaRepository<Category, Integer> {
    ...
    Category updateCategory(@Param("category_id") Integer category_id, @Param("category") String category);

    @Modifying
    @Transactional
    @Query(value = "UPDATE category SET status = 1 WHERE category_id = :category_id", nativeQuery = true)
    Category activateCategory(@Param("category_id") Integer integer);

    @Modifying
    @Transactional
    @Query(value = "UPDATE category SET status = 0 WHERE category_id = :category_id", nativeQuery = true)
    void deleteById(@Param("category_id") String category_id);
